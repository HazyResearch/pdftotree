...
from pdftotree.utils.pdf.pdf_parsers import (cluster_vertically_aligned_boxes,
                                             get_char_width,
...
def get_alignment_features(line_bboxes, elems, font_stat):
    ...
    for line_bbox in line_bboxes:
        line_bbox_ordered = (line_bbox[4], line_bbox[3], line_bbox[6],
                             line_bbox[5])
        boxes = [
            elem for elem in elems.mentions
            if intersect(line_bbox_ordered, elem.bbox)
        ]
        boxes_segments = [
            elem for elem in elems.segments
            if intersect(line_bbox_ordered, elem.bbox)
        ]
        boxes_figures = [
            elem for elem in elems.figures
            if intersect(line_bbox_ordered, elem.bbox)
        ]
        boxes_curves = [
            elem for elem in elems.curves
            if intersect(line_bbox_ordered, elem.bbox)
        ]
        ...
        nodes, nodes_features = cluster_vertically_aligned_boxes(
            ...
            boxes_segments, boxes_curves, boxes_figures, page_width, True)
